...
12. **Non-prose text before graphbrain: script built and tested (2026-09-25), not yet run on the corpus (stage P1b).** Table rows, LaTeX,
    formulas and statistics are removed by `nonprose_removal_v1.py`; 85 hard cases in its self-test pass, and on 600 fresh articles no line
    count changed and every cleaned line is the masked line with removals only. Known limits: prose lines that contain several "=" signs
    (about 0.4% of lines) and a few number lists (DOIs, checksums) remain; captions with a sentence, URLs and the symbols x, degree, micro
    and square are kept on purpose; table rows are found from the raw text, so the script needs the raw files. A row with a single tab-separated
    cell is treated as prose.
...
| `pmc_preprocessing/nonprose_removal_v1.py` | 098d19ecbfde | 2026-09-25 22:14 |
