...
| F1 | Article blacklist: articles removed from the analysis, one row per article and reason (batch 1: paper type; batch 2: preprints; batch 3: PubMed lookup for the unclassified; batches 4 and 5: duplicate resolution, see F2; later batches: manual) | `python3 pmc_preprocessing/build_article_blacklist.py` (batch 1, refuses to overwrite); `python3 pmc_preprocessing/append_blacklist_preprints.py` (batch 2, backs up and appends); `python3 pmc_preprocessing/fetch_pubmed_types_unclassified.py` (network, key from `NCBI_API_KEY`), then `python3 pmc_preprocessing/refine_blacklist_unclassified.py` (batch 3, backs up and rewrites) | `LCS/target_pmcids.txt`, `LCS/target_metadata.json`, `LCS/pure_text_corpus/` (Subjects, PMID and Article version lines), `PP/sequence_metadata_typed_v2.csv` (May PubMed types), PubMed esummary | `LCS/article_blacklist.csv` (2,598 rows after batch 5: preprint 1,475, case report 377, no PubMed record 252, correction/erratum/retraction 111, guideline/consensus 21, address 1, duplicates 361); `PP/pubmed_types_unclassified_20260925.csv`; `LCS/article_blacklist_removed_20260925.csv` (952 articles found admissible and removed); backups `LCS/article_blacklist.before_preprints_20260925.csv`, `LCS/article_blacklist.before_unclassified_refine_20260925.csv`, `LCS/article_blacklist.before_duplicates_20260925.csv`, `LCS/article_blacklist.before_duplicates_v2_20260925.csv` | 2026-09-25 13:46 to 14:03 [LOG] | DONE-current (not yet applied by any later stage) |
| F2 | Duplicate screen and resolution. Screen: character-level Levenshtein distance between article titles (normalised: lower case, letters and digits only; divided by the longer title) for all articles not on the blacklist. Resolution: each close pair is decided by PubMed links, title patterns, PubMed types, authors and the similarity of the openings; the chosen articles go on the blacklist (F1, batch 4) | `python3 pmc_preprocessing/title_levenshtein_duplicates.py --workers 8` (about 1 min), then `python3 pmc_preprocessing/resolve_duplicate_pairs.py` (dry run) and `python3 pmc_preprocessing/resolve_duplicate_pairs.py --apply --intro-threshold 0.5` (batch 4) and `... --apply --tag v2 --intro-threshold 0.5` (batch 5, widens the link, no-record and author rules from title distance 0.15 to 0.30); network: PubMed, key from `NCBI_API_KEY`; backs up and appends | `LCS/target_pmcids.txt`, `LCS/target_metadata.json`, `LCS/article_blacklist.csv`, `PP/content_regions_v2.csv`, texts, PubMed efetch | `LCS/title_screen/title_pairs_v1.csv` (825 pairs at distance 0.30 or less), `title_clusters_v1.csv`, `title_screen_summary.json`, `duplicate_decisions_v1.csv` and `duplicate_decisions_v2.csv` (one row per pair with category, decision and evidence), 293 and 68 rows appended to `LCS/article_blacklist.csv` | 2026-09-25 14:13, 14:37 and 14:48 [LOG] | DONE-current |
...
| Blacklisted (F1/F2, 2026-09-25) | 2,598 | of 46,177 texts: paper type or no PubMed record 762, preprints 1,475, duplicates 361; the list is meant to grow |
...
| F2 | `python3 pmc_preprocessing/title_levenshtein_duplicates.py --workers 8`; `python3 pmc_preprocessing/resolve_duplicate_pairs.py --apply --intro-threshold 0.5`; `... --apply --tag v2 --intro-threshold 0.5` | `[LOG]` RL-035, RL-037, RL-039 |
...
| `pmc_preprocessing/resolve_duplicate_pairs.py` | 5a53f637e7eb | 2026-09-25 14:48 |
