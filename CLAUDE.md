...
diagnostics keys:
    ...
    "raw_sparsity_loss"  float   unweighted sparsity_loss (ticket 78)
    "raw_domain_balance_penalty" float unweighted in-loop domain-balance
                                 penalty (ticket 82 E2) — always computed,
                                 nonzero even though its weight
                                 (lambda_domain_balance) defaults to 0.0

evaluate_complete_solution(...)
    -> dict with EXACTLY these keys (all native Python float, see ticket 68;
       "domain_balance_pen"/"mean_dev_k" added ticket 82 E2):
       "collapse_pen", "collapse_score", "coherence_pen",
       "weakest_coherence", "semantic_pen", "domain_balance_pen",
       "mean_dev_k", "sociological_penalty"
    sociological_penalty = collapse_pen + coherence_pen + semantic_pen +
    domain_balance_pen — domain_balance_pen is summed in UNCONDITIONALLY,
    active regardless of the in-loop lambda_domain_balance weight (mirrors
    collapse_pen/coherence_pen, neither of which has an in-loop counterpart).
    ...
    / evaluate_domain_balance. This is the single source of truth — call it,
...
> **[SUPERSEDED IN PART — corrected 2026-09-22 against the code, not rewritten, so the
> original diagnosis stays visible.] The sum has FOUR terms, not three.** Ticket 82 E2 added
> `domain_balance_pen`, summed in unconditionally: `sociological_penalty = collapse_pen +
> coherence_pen + socio_semantic_pen + domain_balance_pen` (`chunk13v9.py` ~line 1521,
> verified directly). The "one term wearing three names" framing below therefore describes
> the pre-E2 state. It is not simply repaired by adding a fourth name: FINDINGS §25 finds
...
### 4.18 Per-community domain balance — mechanism IMPLEMENTED (E2), its input now in
question (E3); ticket 82

> **HOW TO READ THIS SECTION (added 2026-09-22).** It was written as a design document when
> nothing was implemented, and the "IMPLEMENTED" blocks (E2, then E3) were appended beneath
> without revising the original text. Three claims in the design narrative below —
> "v9 currently has no per-community domain-balance mechanism at all", "not yet implemented
> in code", and the `Z_scaled`-based outer-loop bullet — are **historical, not current**, and
> each is flagged in place. **The current state is:** both mechanisms exist in
> `chunk13v9.py`; the in-loop weight is `0.0`; the outer-loop `evaluate_domain_balance` is
> `U_prob`-based (NOT `Z_scaled`) and is summed into `sociological_penalty` unconditionally;
> and per FINDINGS §25 the quantity it reads carries no signal about true domain balance on
> this corpus, though the formula itself is correct. Read E2 and E3 at the end of this
> section before acting on anything in the design narrative.
...
**[HISTORICAL — false since ticket 82 E2. v9 has two: an in-loop differentiable term in
`run_inner_solver` (weight `lambda_domain_balance`, default `0.0`) and the outer-loop
`evaluate_domain_balance`. Kept as the record of the state this design responded to.]**
...
update).** **[HISTORICAL — "not yet implemented in code" was true when written; both halves
are implemented, see E2 below.]** v8's relation-level framing (and the naive `U_scales/√N_f` port of it) forced an
...
- **Outer-loop (`Z_scaled`-based, mirrors `collapse_pen`'s construction):** **[SUPERSEDED —
  DO NOT BUILD THIS. The shipped `evaluate_domain_balance` is `U_prob`-based, not
  `Z_scaled`-based; its signature is `evaluate_domain_balance(U_prob, presence_masks, tol)`.
  A `Z_scaled` version of exactly this idea was tested and found EXPLOITABLE via ticket 79's
  gauge freedom — its proxy improved 30-40× while real membership imbalance worsened in 3 of
  12 cells (see E2 below and FINDINGS §22). This bullet is the rejected draft, kept so the
  rejection is legible.]** per relation,
...
`diagnostic_blocks.py` 1.12.0) answers the "unanswerable" question below — GO.** Equal-weighted
pilot: mean `dev_k` (deviation from `r_k=0.5`) ≈0.08 both slices, no consistent direction
(26/25/25 social/semantic/balanced split), ~31-32% of communities exceed a 40/60 band. **Now
re-tabulated with entity weighting as primary (FINDINGS §13's D2 update)** — materially
larger and directionally skewed: mean `dev_k` 0.13 (T1) / 0.13 (T2) in `u_prob` space, 57-62%
of communities exceed a 40/60 band, and the direction is no longer even (28 semantic-leaning
vs 17 social-leaning of 45 skewed, pooled). Traced to composition, not noise alone: `auth` is
~75% of the entity-weighted social pool in both slices (confirmed `soc_k` tracks `auth`'s own
...
§21. **Re-derived on post-08-27-fix data (tickets 86/87 Stage 0e): verdict held, `TOL`
kept at `0.15`.** Both the noise floor and the `dev_k` signal shrank on the post-fix data
(noise median 0.144→0.120, `dev_k` median 0.112→0.093) — the margin between `TOL` and the
noise median widened, not narrowed, and the fraction of communities whose imbalance clears
their own noise floor dropped at every multiple tested (46%→36% at 1×, 13%→4.8% at 2×) —
if anything more of the apparent signal reads as noise now, reinforcing the original
caution. Deliberately not re-tuned upward toward the new p90 either — see FINDINGS §21 for
why. Re-run scripts, no code changed.

**E2 — IMPLEMENTED (both in-loop and outer-loop; "Path B"), weight fixed at `0.0` for the
toy corpus. Full evidence: FINDINGS §22.** Both mechanisms are now real code in
`chunk13v9.py`, not diagnostic-only:

- **In-loop** (`run_inner_solver`): `lambda_domain_balance = params.get('lambda_domain_balance',
  0.0)` — read the same way `lambda_l1` is post-ticket-78, **not** in Optuna's search space.
  The differentiable term itself (`FACET_DOMAIN`-grouped, entity-weighted, live-masked
  `U_prob` means → `r_k` → squared dead-band past `DOMAIN_BALANCE_TOL=0.15`) is computed every
  epoch regardless and exposed unweighted as `diagnostics["raw_domain_balance_penalty"]`
  (mirrors `raw_sparsity_loss`).
- **Outer-loop** (`evaluate_complete_solution`, Module 3): a **new** function,
  `evaluate_domain_balance`, computes the same formula post-hoc on the final `U_prob` and is
  summed into `sociological_penalty` **unconditionally** — this already changes Optuna's
  second Pareto axis today, independent of the in-loop weight, the same architecture
  `collapse_pen`/`coherence_pen` already use (neither has an in-loop counterpart either).

**Why `U_prob`, not `Z_scaled`, for both halves — a concrete, measured reason, not just a
preference.** A separate, pre-existing exploratory script already in `diagnostic_blocks.py`
(`make_domain_balance_penalty_torch`, undated, found this session while building E2) tested a
`Z_scaled`-based version of this same idea and found it **exploitable**: its own optimized
proxy (`mass_tvd`) improved 30-40× in every one of 12 tested cells while the real
`U_prob`-based membership imbalance (`mem_tvd`) got **worse** in 3 of 12 — the optimizer was
gaming `Z_scaled`'s undetermined `U_scales` gauge freedom (ticket 79), not fixing real
imbalance. `U_prob` is proven immune to that exact freedom, and this was reconfirmed
directly: the new `evaluate_domain_balance`'s output matched the independently-built
diagnostic pipeline to ~8 significant figures on a real fit.

**Why the weight is `0.0`, not some nonzero value — measured, not guessed, and not merely
"conservative."** The formula's raw magnitude was measured on real baseline fits first (the
same first step ticket 77 used for `lambda_l1`) — it does not transfer cleanly: because the
term is a dead-band (0 below TOL, unlike `mean_sparsity`'s always-positive value), 5 of 12
baseline fits gave an **exact** 0.0, making a `lambda_l1`-style range derivation ill-defined.
Testing across weight ∈ {0.5,1,2,5,20,50,200,400} (including a rescaled higher-exponent
variant), both `chunk12` and `chunk12v2` data, multiple pilot cells, multi-seed: **only 1 of
20 (weight/power/cell) combinations tested produced a shift exceeding its own paired
seed-noise floor**, and `weight=5` caused real non-convergence (down to 1/5 seeds converging
in one cell) at a loss-magnitude fraction (~2.7% of `math_loss`) a naive derivation would not
have flagged as risky. Full numbers, all tables: FINDINGS §22.

**E3 — `dev_k` CARRIES NO SIGNAL ABOUT TRUE DOMAIN BALANCE ON THIS CORPUS (negative result,
FINDINGS §25). The FORMULA IS NOT AT FAULT — the input is.** An audit of all three
implementations found them correct and mutually consistent (in-loop torch / outer-loop numpy
/ `diagnostic_blocks` agree to 7.5e-10 on `C6/K=4/T1`), and gauge-invariant as designed
(`mean_dev_k` moves by exactly 0.00e+00 under ticket 79's transformation). The fault is that
the fitted community assignments these functions read do not identify the communities that
generated the data, which originates in corpus sparsity, upstream of `chunk13v9.py`
altogether. Do not read the below as "the measure is invalid"; read it as "a faithful reading
of an input that does not identify the communities."

A planted-structure test (synthetic
data carrying this corpus's exact degree sequence and weight distribution, passed through the
real `run_inner_solver`; 180 fits) finds `dev_k` **flat with respect to the truth it is meant
to measure**: a world whose communities are exactly balanced by construction reports mean
`dev_k` ≈ 0.103, a world containing a 70/30 community reports ≈ 0.096, and a world containing
a **90/10** community reports ≈ 0.101. Detection of a genuinely mono-domain community, against
the balanced arm's own 95th percentile, is 6–22% — at or barely above the 5% false-positive
rate. On T1, 56–67% of *perfectly balanced* synthetic worlds report more imbalance than the
real T1 data does.

**Mechanism:** the fitted communities do not correspond to the planted ones. Hungarian-matched
recovery accuracy is 0.31 (social) / 0.39 (semantic) at K=4 against a 0.25 chance level, and
0.38 / 0.45 at K=3 against 0.33 — even when 90% of generated ties are within-community.
Degree sparsity is the background condition: live-entity total degree has median 2, and
**44–45% of live entities have a total degree of exactly 1** (verified — the minimum over
live entities is 1; degree 0 cannot occur, since "live" is defined as having at least one
non-zero entry), rising to 50–55% among social entities. Degree alone is *not* claimed as the
mechanism: recovery by degree bucket is non-monotone and reverses between cells, and the
recovery metric is argmax-based, a crude instrument against this model's soft partition. The
consistent signal is the domain asymmetry, not a degree law. Fitted
sharpness is 0.81 (social) / 0.82 (semantic) — the model is equally *and spuriously* confident
in both domains, which is also why an earlier post-hoc null (sharpness-based, the user's
original hypothesis) found nothing: sharpness is equal between domains, and equally
uninformative.

**Direct consequences, not yet acted on in code:** (1) `evaluate_domain_balance` is summed
into `sociological_penalty` unconditionally and therefore shapes Optuna's second Pareto axis
*today*, on a quantity this test says does not track what it names — the in-loop weight being
0.0 does not protect against that; (2) an outer-loop veto on "obviously broken" communities
cannot be built on `dev_k` (a 90/10 community is caught 6–22% of the time); the parallel
candidate reading community *mass* (`max_share`) is a different quantity and is untested — a
planted test for it is proposed, not run. **Limitations, stated because they bound the
claim:** ties are degree- and weight-preserving but not motif-preserving (real co-authorship
cliques are not reproduced), one model seed, three configs, K∈{3,4}. Re-derive at 22k-article
scale, where the degree sparsity driving the failure should ease substantially.
...
**Definition (user's, from a discussion external to this project's own record): a community
is a candidate "ghost" primarily by very low share of the model's total reconstructed mass.**
Domain skew and facet concentration are secondary, descriptive properties of a low-mass
community, not part of the definition — measured below to often be *absent* even when a
community is low-mass, which is exactly why the user's own framing keeps them secondary.

**Mass measure: `community_share`, already computed (unexposed) inside
`evaluate_dimensional_collapse`** — the full length-K vector `max_share`/`collapse_pen` are
drawn from, per-relation shares (`_relation_community_share`) averaged and renormalised.
Validated directly for the two specific risks this design is exposed to, not assumed safe:
**exactly gauge-invariant** under ticket 79's transformation (compensated case moved it by at
most 4.5e-8, the same order as ordinary float32 round-off; an uncompensated control moved it
by 0.06–0.24, confirming the test has real teeth) and **unaffected by dead-entity
contamination** (zeroing dead rows changed it by exactly 0.0 in every cell tested, explained
by dead-row magnitudes sitting 5+ orders of magnitude below live-row values). Two older,
separate limitations of anything built on `Z_scaled` are **not** closed by this: general
rotational indeterminacy (§1) and per-relation permutation beyond the Hungarian correction's
trust (FINDINGS §14) — both remain open, and FINDINGS §18's existing evidence that blending
freedom is essentially absent covers only K∈{3,4}, T1 — this ticket's grid is wider
(K∈{2..6}, T1/T2, both `chunk12`/`chunk12v2` data) and that check has not yet been extended
to match.
...
- `S_Art_Auth`'s within-community diagonal mass concentrates on far fewer than K communities
  **in every one of the 6 configs, at every K from 2 to 6, both slices, both data
  versions** — top-2 share averages 0.77–1.00 against a fair share of 0.33–1.00 depending on
  K. Not ghost-specific; a general property of this relation, most likely (not yet proven)
  because it has only 218–258 total non-zero ties (§11) — too little data to differentiate
  more than ~2 communities' worth of author-article pattern.
- **`auth`/`affil` collinearity (FINDINGS §4, now extended past its previous K≤4 ceiling to
  K=6) explains this in `C1` specifically (mutual collinearity 0.70–0.82, both data
  versions) but not in `C3` (collinearity ≤0.011 in every version tested)** — the same
  overall symptom (low share) has at least two distinct causes across configs, confirmed by
  an independent affiliation-removal pilot: excluding `S_Auth_Affil` from the fit entirely
  measurably improved `C1`'s `S_Art_Auth` resolution and lowered its collinearity, but left
  `C3`'s ghost community at nearly the same share, same community index, essentially
  unchanged.
- A "mega-author-count paper drives the winning communities" hypothesis was tested directly
  (article degree vs. winner-community loading, all 6 configs, K=6, both slices) and **not
  supported** — correlations mostly weak/non-significant, the one significant result runs
  opposite to the hypothesis.
- Flagged communities are usually **not** content-starved on the article side — in 4 of 6
  cells checked, at least one flagged community has a top-2 (often the single strongest)
  article-side membership in the whole fit; the deficit is specifically that this doesn't
  translate into `S_Art_Auth` diagonal mass, pointing at the author side, not the article
  side.
- **Loss-weight sensitivity checked, not assumed:** outer-loop penalties cannot confound
  this (structurally excluded from training, §4.1). `lambda_z_offdiag` does — collinearity
  magnitude is materially inflated by it (C1: 0.263 at λz=0 → 0.699 at 0.05 → 0.865 at 0.5),
  though the qualitative C1-vs-C3 ordering and the `S_Art_Auth` concentration finding itself
  both survive at every tested `λz`. Single-seed, 2-cell check — not a full sweep.

**No mechanism (in-loop or outer-loop) has been designed.** This ticket is diagnostic-only
throughout, deliberately, matching how tickets 82/84 were sequenced. **Not yet done:** the
full affiliation-removal grid (user-requested, all 6 configs × K∈{2..6} × both data
versions, only a 2-cell pilot exists); extending the rotation-feasibility (§18) and
permutation-conflict (FINDINGS §14) checks to this ticket's wider K/slice/data-version grid
(both currently stop at K∈{3,4}/K∈{2,4}, T1 only); a seed-noise floor for `community_share`
(needed before any single cell's "ghost" verdict can be trusted against ordinary run-to-run
variation, the same discipline `TOL` went through, ticket 82 D3). Full evidence, every table,
every script: FINDINGS §23.

### 4.23 Ticket 84 D4 follow-up — journal-resolution false negatives found and mostly
fixed; zero effect on this toy corpus's fitted data; a process-safety incident

**A follow-up to D4 (§4.21) found the original journal-repository resolver had a real bug:
it checked a candidate's `primary_location` only, never its full `locations` list.**
Confirmed directly against T5's OpenAlex record, which lists JMLR as a non-primary
location. Of 6 well-known ML papers checked in the "49 unresolved" bucket, 5 remain
genuinely unresolved after the fix — their OpenAlex records really do list only arXiv, a
data-coverage gap the fix cannot close — and 1 (T5) now resolves correctly. Two further
self-caught bugs during the fix: the repository check only recognized 4 named IDs (missed
PubMed and DROPS, both now found and excluded via OpenAlex's own `type=="repository"` field
generally, not a hardcoded ID list); and one title in `articles.csv` contained a literal
`\n` (backslash + letter n, not a real newline) that broke exact-phrase search for that one
article specifically.

**A `venue_type` column was added to `articles.csv` for all 121 articles** (one OpenAlex
Sources lookup per unique journal, not per article), and a persisted, growing reference
(`toy_large/known_repository_sources.json`, 6 entries) now backs `chunk12.py`'s/
`chunk12v2.py`'s `REPO_JOURNAL_IDS` — both files load it at runtime instead of a hardcoded
4-ID literal. **Scoped explicitly to what this corpus has encountered, not a claim about
OpenAlex's complete repository universe** — flagged directly after an early presentation
risked being read that way.

**Both files rebuilt; the fix has zero effect on the fitted matrices.** Full null-rebuild
check: every relation, every map, `journal_meta` — byte-identical before/after, both slices,
both versions. Traced why: none of the 3 corrected articles are part of this corpus's
61-article valid cross-section (`chunk12.py`'s own `meta_set ∩ edge_set ∩ sqlite_set`
filter) — all 3 fail `sqlite_set` membership (absent from `corpus_curated.sqlite`)
regardless of journal.

**Traced further, on request: why.** All 3 have real abstracts (`corpus_text.csv`) and were
successfully graphbrain-parsed (`corpus_raw.sqlite`, 17 sentences total across the 3). Every
one of those 17 sentences failed `postprocessing_4h.py`'s `extract_dual_matrices()`
core-structure requirement — a real, structural filter result, not a parsing or
text-availability failure. Also found: `corpus_curated.sqlite` was built by
`postprocessing_4h.py`; a corrected version, `7.5postprocessing_4hbased_correct.py`, exists
but is dated a day later and was never used to rebuild the production curated database —
untested whether its corrections would change this outcome.

**Process-safety incident, fully recovered:** isolating `postprocessing_4h.py`'s function
definitions by `exec()`-ing everything before its `# EXECUTION` marker inadvertently
triggered a module-level purge of `corpus_curated.sqlite` that lives *before* that marker.
Destroyed, then recovered byte-for-byte from a NAS daily snapshot
(`Reynolds_HUM_NFS_28days_2026-08-04`), verified against the pre-incident state (177
`source_core` edges, 66 articles). No data lost. New standing rule added,
`SESSION_PROTOCOL.md` §E — see there.
...
| 60 | M3 §2/§4, M1 §1.8 | **New.** Entity index maps are GLOBAL across chunk12 time slices; only `art` is slice-specific (§11). A slice's matrices are padded with structurally-zero rows/cols for entities belonging to the other slice — at least 53–61% of authors/parent-hyperedges/etc. in T1 (§11). Collapse mass and monopoly/baseline calculations averaged over ALL rows including dead ones. | **Closed.** `build_presence_masks()` added (M1 §1.8); threaded into `evaluate_dimensional_collapse` (live-count normalization) and `evaluate_socio_semantic_reality` Parts A/B (baseline + monopoly scan restricted to live rows). **[SUPERSEDED — 2026-09-23]** the `evaluate_dimensional_collapse` half is no longer accurate: that function was rewritten on `Z_scaled` (tickets 79/80, FINDINGS §17) and its own docstring now says directly "No presence_masks / live-entity normalisation needed here... every relation's input is already Frobenius-normalised" — matching this file's own historical note near §4.4 ("kept in place; superseded as 'the fix' by the larger finding"), which this register row was never updated to reflect. The `evaluate_socio_semantic_reality` Parts A/B half is unaffected and still current (verified, chunk13v9.py L1256-1289). |
...
| 85 | M2, `initialize_tucker_adapted_nndsvd_and_propagate` (lines ~351, ~524) | **New — found this session while bit-exact-verifying ticket 82's E2 addition, unrelated to it.** Iterates `active_facets` — a raw Python `set()` — directly, which §4.9 already names as exactly this hazard for a different function ("Never iterate a raw `set()`... unordered iteration produces non-deterministic tensor geometry"). Confirmed, not just suspected: `fit_production('C6', 4, ...)` at the same seed gave *different* `math_loss` across separate process launches (bit-identical *within* one process — ticket 76's single-threading fix is intact and unaffected) — fixing `PYTHONHASHSEED` across launches made results bit-identical again, confirming Python's hash-randomized `set()` iteration order as the exact cause, not a new source of within-run non-determinism. | **Open, not fixed.** Out of scope for ticket 82; flagged rather than silently worked around. Fix (when picked up): route `active_facets` through `get_required_facets()` or `sorted(active_facets)` at both call sites, matching §4.9's existing rule — a small, mechanical fix, but touches the same function ticket 64's multi-anchor-overwrite fix lives in, so re-run that fix's C6 smoke test alongside any change here. |
...
| 88 | M1 §1.1/§1.2 + M4 §S1 "PHASE 1A" (was lines ~1686, ~1691, ~1695, ~1696) | **New — four constants re-declared at module level in Module 4, duplicating Module 1's.** `DEVICE`, `MASTER_SEED`, `ENTROPY_THRESHOLD` and `TARGET_COHERENCE` each had a second module-level assignment in Module 4's "PHASE 1A: HARDWARE & MLOPS CONSTANTS" block, contradicting §7's "Module 1 owns all constants... delete the later copy". Found by a mechanical `ast` audit, not by a failure — **no observable behavioural difference existed**, and the confusion this looks like was checked for explicitly before touching anything, rather than assumed absent (ticket 57's precedent is that a duplicate-definition report can itself be wrong). Verified directly against the live file, not the audit's prior claim: both copies of each held identical values (`DEVICE`: the byte-identical `torch.device("cuda" if torch.cuda.is_available() else "cpu")` expression, `cpu` on this node with `torch.cuda.is_available()` `False`; `MASTER_SEED` 42/42; `ENTROPY_THRESHOLD` 0.60/0.60; `TARGET_COHERENCE` 0.50/0.50). Every one of the ~20 usage sites resolves to the *second* copy today regardless of where it sits textually — the file is a single top-to-bottom script with no inter-module imports (§2/§7), everything executable is under the one `if __name__ == "__main__":` at the end (ticket 58), and Python resolves a global name inside a function body at call time against the module namespace as it stands then, so a use textually above line 1686 still saw line 1686's value. Checked by AST that the only *module-level* (import-time) reads are `print(... {DEVICE})` (~1687), `def set_seeds(seed=MASTER_SEED)`'s default (~1713, bound at `def`-execution time) and `set_seeds(MASTER_SEED)` (~1742) — all three sit below both assignment sites, so they are unaffected either way. Not two confused entities under one name: one constant re-declared twice, safely consolidatable. | **Closed.** All four later copies deleted; Module 1's declarations are the single source of truth, with a comment left at the deletion site naming the ticket and §7's rule. `MAX_MONOPOLY = 0.85` and `CONFIG_IDS` in the same block were left alone — each is defined exactly once (M1 §1.2's `MAX_MONOPOLY` is commented out, so M4's is not a duplicate). Verified: `py_compile` passes; a re-run of the same AST audit finds each of the four defined exactly once; the module imports cleanly via `diagnostic_blocks.load_module()` with all four reading their Module 1 values (`cpu` / 42 / 0.6 / 0.5). **No production fit was run** — SESSION_PROTOCOL §B forbids assuming `lambda_l1`/`lambda_z_offdiag` when a prompt doesn't state them, and none of the four names is read by the solver's numerics anyway; note the edit changes `chunk13v9.py`'s content hash and therefore invalidates every `fit_or_load()` cache entry (§D), as any edit to this file does. |
...
| 82 | M2, per-community domain balance | No mechanism currently constrains any individual community's semantic/social mix; only the global 50/50 reconstruction-loss weighting exists. v8's `binding_penalty` (per-community, differentiable) was lost in the softplus-era rewrite and not restored. See §4.18. | **Open — E2 IMPLEMENTED in code; the open question is now E3, whether the quantity it reads means anything on this corpus.** *(Status line corrected 2026-09-22: this cell previously opened "implementation not yet started" while also containing "E2: IMPLEMENTED" further down — self-contradictory. Implemented is correct; the D1-D3 design record that follows is kept unchanged.)* D1-D3 decided. The relation-level anchor-double-counting problem this row previously described is **superseded**, not solved in place — a facet-level formulation (domain is a facet property, not a relation property) makes it not arise: verified `art` has exactly 2 non-anchor relations in every config regardless of anchor count, so no anchor-count correction is needed. E1 answers the prior "currently unanswerable" question in §4.18 below: real per-community imbalance exists on a determined basis. **D2 (weighting): entity, re-tabulated** — materially larger and directionally skewed toward semantic-leaning vs. the equal-weighted pilot, traced exactly (not just suspected) to `auth`'s ~75% share collapsing the social-side averaging from 4 facets to ~1.7 (Herfindahl/Kish derivation, FINDINGS §21, near-exact match to observed spread). **D3 (`TOL`): decided at `0.15` for the toy corpus**, set from a measured multi-seed noise floor (median 0.144, comparable to the `dev_k` signal's own median 0.112) rather than the entity-weighted spread directly — FINDINGS §21; flagged for re-estimation at 22k-article scale. **Re-derived post-08-27-fix (tickets 86/87 Stage 0e): verdict held, kept at `0.15`** — noise floor and signal both shrank (median 0.144→0.120 / 0.112→0.093), widening rather than narrowing `TOL`'s margin; not re-tuned upward either, see FINDINGS §21. **E2: IMPLEMENTED** — both an in-loop differentiable term (`run_inner_solver`, weight `lambda_domain_balance` fixed at `0.0`, not Optuna-tunable — matches `lambda_l1`) and an outer-loop check (`evaluate_domain_balance`, new, summed into `sociological_penalty` unconditionally, active regardless of the in-loop weight) — both built on `U_prob`, not `Z_scaled`, after a separate pre-existing exploratory script's `Z_scaled`-based candidate was found exploitable via `U_scales`'s undetermined gauge freedom (ticket 79) — its own optimized proxy improved 30-40x while real membership imbalance worsened in 3/12 tested cells. Weight tested across {0.5,1,2,5,20,50,200,400} (incl. a rescaled higher-exponent variant), both `chunk12`/`chunk12v2` data: only 1/20 combinations cleared its own seed-noise floor, `weight=5` caused real non-convergence (down to 1/5 seeds). See §4.18 and FINDINGS §22 for the full evidence. **E3 (tickets 86/87 Stage 0e follow-up): `dev_k` carries no signal about true domain balance on this corpus — FINDINGS §25. The FORMULA IS NOT AT FAULT; the input is.** A planted-structure test (180 fits, synthetic data with this corpus's real degree sequence, real solver) finds `dev_k` flat against the truth: exactly-balanced worlds report ≈0.103, worlds containing a 90/10 mono-domain community report ≈0.101, detection 6-22%. A subsequent line-by-line audit found all three implementations correct, mutually consistent (7.5e-10 on `C6/K=4/T1`) and gauge-invariant as designed (0.00e+00 under ticket 79's transformation) — so this is a faithful reading of an input that does not identify the communities, not a defective formula, and the defect originates in corpus sparsity upstream of `chunk13v9.py`. Cause: fitted communities do not correspond to planted ones (recovery 0.31 social / 0.39 semantic at K=4 vs 0.25 chance), against a background of 44-45% of live entities having a total degree of exactly 1 (degree 0 cannot occur — "live" means at least one non-zero entry), rising to 50-55% among social entities; degree alone is not claimed as the mechanism, since recovery by degree is non-monotone and the argmax-based recovery metric is crude against a soft partition. The user's own sharpness hypothesis was tested separately and refuted (social/semantic row-max 0.80 vs 0.81; equalising changes the expectation by ≤0.002). **Note the outer-loop term is live on this signal today** (summed into `sociological_penalty` unconditionally). Toy-corpus scoped; motif structure not reproduced; re-derive at 22k scale. **Permutation-correction groundwork corrected and shipped:** FINDINGS §16's original "1 solid real conflict" (C5/K4 `art`) was itself a bug — the confidence re-filter that produced it forgot to re-apply leaf-exclusion, wrongly counting `S_Art_Journ` (touches the leaf facet `journ`) as a dissenting vote. Corrected: **zero confirmed real non-leaf conflicts anywhere in the grid.** `evaluate_dimensional_collapse` therefore corrects each relation independently and does not implement cross-relation conflict resolution — documented limitation, see FINDINGS §17. Criterion chosen (`structure_score > 1.0`) verified head-to-head against `chunk13v3.py`/`v4.py`'s original `diagonal_mass/hungarian_mass < 0.7` — 12/94 disagreements, 8 of them cases the older criterion would have wrongly "corrected" a genuinely mixed relation. **Toy-corpus calibrated throughout — re-derive at 22k-article scale before reuse.** |
| 86 | M2/M3, per-community mass ("ghost communities") | No mechanism exists to detect or address a community holding very low share of the model's total reconstructed mass — the third major open issue this session, alongside tickets 82 (domain balance, frozen at weight 0) and 78 (`lambda_l1`, frozen at 0), both pending 22k-scale re-derivation. User definition: primarily a mass-share property; domain/facet concentration are secondary, descriptive properties, not part of the definition. | **Open — measurement phase only, no mechanism proposed.** `community_share` (already computed inside `evaluate_dimensional_collapse`, previously only exposed as `max_share`) validated as the mass basis: exactly gauge-invariant under ticket 79's transformation (compensated case moved it ≤4.5e-8, an uncompensated control moved it 0.06–0.24, confirming the check has teeth) and unaffected by dead-entity contamination (exactly 0.0 change in every cell tested). A flat 2% mass threshold never fires anywhere in the full C1–C6×K∈{2..6}×T1/T2 grid (either data version); a `K`-relative threshold (`0.5/K`) does, rising with K, with 3 cells showing 2 simultaneous low-mass communities. Root cause: `S_Art_Auth`'s diagonal structurally concentrates on far fewer than K communities in every config/K/slice/data-version tested (likely its own data sparsity, 218–258 total ties, §11) — not ghost-specific. `auth`/`affil` collinearity (FINDINGS §4, extended to K=6) explains this in `C1` specifically (confirmed both data versions) but not `C3` (ruled out both data versions) — confirmed by an independent affiliation-removal pilot with a matching config split. A "mega-author-paper drives it" hypothesis was tested and not supported. `lambda_z_offdiag` (not outer-loop penalties, which are structurally excluded from training) materially inflates the measured collinearity magnitude, though not the qualitative config split or the `S_Art_Auth` concentration finding itself. See §4.22 and FINDINGS §23 for the full evidence. |
| 84 | chunk12.py, `compile_slice`/`build_*_csr` (upstream data generator, not `chunk13v9.py`) | `S_Art_Journ` and the 5 grammar relations (`M_Atom_Child` etc.) currently get no frequency/degree weighting at all, unlike `S_Art_Auth` (log-damped, ticket 63) and the 3 anchor relations (tf-idf weighted). Measured degree concentration in all 5 grammar relations is comparable to or worse than `S_Art_Auth`'s already-damped case — `M_Atom_Child` (13.0×/16.6× row max÷mean, T1/T2) is the worst in the whole topology, currently unaddressed. Connects to §9's already-documented, unresolved "ubiquitous semantic elements... structurally disfavoured" limitation. | **Phases 1-4 executed on the toy corpus; new versioned data exists but is NOT the production default.** Full record: `PhD_CNMF/plans/ticket84-grammar-hub-downweighting.md`. Design (Opus) then independent triangulation (Sonnet) settled D1-D8: **damping formula** — reuse `build_log_damped_row_csr` verbatim (Counter-of-1s), zero new formula; **scope (D8, revises the original "all 5" framing)** — only `M_Atom_Child`/`M_Fringe_Cousin` (atom-rooted, same ubiquity argument as `S_Art_Auth`) and `M_Cousin_Child` (bespoke justification — row degree there measures *context genericness* for interpretation, not ubiquity; verified `dummy_cousin` predicate structures are 2× overrepresented in the high-degree tail) damped; `M_Child_Parent`/`M_Cousin_Parent` left untouched (recurrence there plausibly reflects genuine synthesis, not noise); **`S_Art_Journ` deferred entirely** (D4 — needs new `dfreq_global['journ']` collection + a column-keyed idf variant, contra the original briefing's "low risk" claim, corrected in §4.21 above). **D5 (§9 link): RESOLVED, user-confirmed — struck.** `U_prob`'s L1 row-normalization discards row scale, so a row-rescaling mechanism cannot address a row-*shape* limitation; both independent passes recommended removal, no dissent, user confirmed. This only removes the claim that ticket 84 addresses §9 — §9 itself (§9 below) is untouched and remains its own, separately unresolved pipeline limitation. Phase 1 (Tier-0, read-only): **PASS**, all 6 metrics, degree-1 fragmentation-guard ceiling set from measurement (91-96% of population share, not guessed). Phase 2: `diagnostic_blocks.py` `_cache_key` now includes an input-data content hash, namespaced not invalidating (379 pre-existing cached fits untouched). Phase 3 (paired-seed fit validation, 240+ fits, corrected for a real cross-config-pooling bug found mid-session — see the plan file): **genuinely mixed** — 3 of 6 target (facet, slice) cells moved the predicted direction, 3 didn't; 2 of 6 clear the seed-noise floor with statistical confidence, one in each direction. **Held, not chased further on this corpus** (user decision) — 2 of 3 wrong-direction cells don't clear the noise floor (plausibly just noise); the one exception is small; the noise floor itself is comparable to the effect size throughout (consistent with FINDINGS §21). **`cousin_he`/T2 precision follow-up (tickets 86/87 Stage 0e): resolved.** Its "confirmed" status was the more fragile of the two, resting on a 5-seed SD estimate on the thinnest sample (`M_Cousin_Child` active in only 2/6 configs). Extended to 15 seeds and re-run on post-08-27-fix data: Δρ = −0.242, SD tightened to 0.113 (not widened), now clears 2×SD comfortably (≈2.15×) — on the old pre-fix data the same extension had instead pulled it *below* the bar (ratio fell to 1.7×), so this specifically closes that fragility rather than just re-confirming it. Headline Phase 3 tally unchanged (3/6 correct direction, 2/6 confirmed). Full numbers: FINDINGS §21. Phase 4: `chunk12v2.py` created (copy of `chunk12.py`, `chunk12.py` itself untouched — verified nothing references it by filename), writes to new `_v2`-suffixed pickles (production pickles never touched), null-rebuild verified clean (maps identical, all untouched relations byte-identical, all 3 damped relations match the validated transform exactly, both slices). `diagnostic_blocks.SLICE_PATHS` gained additive `'T1_v2'`/`'T2_v2'` entries — **not** the default `'T1'`/`'T2'`. **Whether `_v2` data ever becomes production is a separate, later, not-yet-made decision.** One named open item for the 22k-scale rebuild: `fringe_atom`/T2's wrong-direction result isn't explained by T1-being-thinner alone (it's the *better*-resourced slice) — flagged for a specific re-check, not just a re-run of the aggregate test. **Follow-up (D4 journal-resolution correctness):** a real bug in the original resolver (checked only `primary_location`, not the full `locations` list) was found and fixed; 1 of 49 previously-unresolved articles corrected (T5→JMLR), the rest confirmed genuinely unresolvable (an OpenAlex data gap, not a script defect). `venue_type` column added to `articles.csv`; `known_repository_sources.json` created; both `chunk12.py`/`chunk12v2.py` now read `REPO_JOURNAL_IDS` from it. Rebuilt both — verified zero effect on the fitted data, traced to none of the corrected articles being part of this corpus's valid cross-section (absent from `corpus_curated.sqlite`, traced through `corpus_text.csv`→`corpus_raw.sqlite`→postprocessing, where all 17 of their sentences failed the core-structure requirement). See §4.23 and FINDINGS §24 for the full record, including a process-safety incident (recovered from a NAS snapshot, no data lost) and the new `SESSION_PROTOCOL.md` rule it produced. |
...
| 63 | Confirmed (not a code change): mega-hub damping for article-author ties lives upstream in chunk12's `build_log_damped_row_csr` (chunk12.py:291-307 — **[SUPERSEDED, 2026-09-23: now chunk12.py:324-340 after ticket 84 D4's REPO_JOURNAL_IDS refactor, §4.23; mechanism unchanged, see §11]**), applied to `S_Art_Auth` specifically (chunk12.py:328, now line 360-361). 13v9 does not need its own damping logic. See §11. |
...
after the fact. **[SUPERSEDED — line numbers drifted, 2026-09-23]:** the ticket 84 D4 `REPO_JOURNAL_IDS`/`known_repository_sources.json` refactor (§4.23) inserted ~33 new lines into `chunk12.py` above this point in the file. As of this correction, `compile_slice()` is at chunk12.py:355-376 and the two matrix-payload `pickle.dump()` calls are at chunk12.py:405 and 409. The described behavior (no `'dimensions'` key is ever written to the payload) is unchanged and re-verified directly against the current file -- only the cited line numbers moved. **Module 1's auto-discovery branch in `load_and_validate_data` is the live
...
**`S_Art_Journ`'s nnz was 24/35 as originally measured; it is 13/13 as of the ticket-84 D4
journal-repository-resolution fix** (dropping ties to arXiv/bioRxiv/ChemRxiv/Research
Square where no real venue could be resolved — see §4.21 — later corrected further, see
§4.23). `TOTAL` above is recomputed to match. `REPO_JOURNAL_IDS` (the set of source IDs this
drop applies to) is no longer a hardcoded literal in `chunk12.py`/`chunk12v2.py` — both now
load it from `toy_large/known_repository_sources.json` at runtime (§4.23).
...
parent hyperedges are live" in T1 — now measured exactly: 46.0% and 39.4% respectively). **[SUPERSEDED — 2026-09-23, re-verified directly against the production pickles]:** the `journ` row in the table above (14/20, 70.0% live in T1) is stale — it predates the ticket 84 D4 journal-repository-resolution fix (§4.23), which dropped `S_Art_Journ`'s T1 nnz from 24 to 13 and, with it, the number of distinct live journals. Recomputed directly from `Star_extended_matrices_t1.pkl`: **11/20 live journals in T1 (55.0%)**, not 14/20 — T2's figure (12/20, 60.0%) was already correct. This also revises the summary range above: T1's live fractions now span **39.4–55.0%** (`parent_he` to `journ`), not 39–70% — `journ` is no longer T1's maximum; `fringe_atom` (53.2%) is next highest.
...
Article-Author ties`) **[SUPERSEDED — line numbers drifted, 2026-09-23]:** same ~33-line shift as §11's "No `'dimensions'` key" note above (ticket 84 D4's `REPO_JOURNAL_IDS` refactor, §4.23). `build_log_damped_row_csr()` is now at chunk12.py:324-340; the `S_Art_Auth` call site and its `# [!] UPDATED` comment are now at chunk12.py:360-361. The formula and its S_Art_Auth-only application are unchanged and re-verified directly against the current file and the live production pickles -- only the cited line numbers are stale. — the article→author tie matrix. `S_Auth_Affil` instead uses plain
