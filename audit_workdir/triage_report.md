...
- Citation graph: 359 §N references checked, **0 broken**
...
- Ticket mentions: CLAUDE.md=108, FINDINGS.md=64
- Correction-tag mentions: CLAUDE.md=19, FINDINGS.md=15
- "Planned/not-implemented" status-language hits (raw material for the 1b deliverable): CLAUDE.md=20, FINDINGS.md=15
    - CLAUDE.md:215 ["not yet implemented"] `or the non-monotonicity. Not yet implemented — the mass-input problem (point 1) should be`
    - CLAUDE.md:473 ["not yet acted on"] `> current live behavior. Read the "Practical consequence" and "Not yet acted on" paragraphs`
    - CLAUDE.md:495 ["not yet acted on"] `**Not yet acted on.** Redesigning this is a decision, not a bug fix — see FINDINGS §15 for`
    - CLAUDE.md:506 ["not yet implemented"] `> "v9 currently has no per-community domain-balance mechanism at all", "not yet implemented`
    - CLAUDE.md:536 ["not yet implemented"] `property (design session, not yet implemented in code; full reasoning in FINDINGS §13's`
    - CLAUDE.md:537 ["not yet implemented"] `update).** **[HISTORICAL — "not yet implemented in code" was true when written; both halves`
    - CLAUDE.md:591 ["diagnostic-only"] `**E1 measurement (diagnostic-only, `diagnostic_scripts/domain_balance_measurement.py`,`
    - CLAUDE.md:624 ["diagnostic-only"] ``chunk13v9.py`, not diagnostic-only:`
    - CLAUDE.md:696 ["not yet acted on"] `**Direct consequences, not yet acted on in code:** (1) `evaluate_domain_balance` is summed`
    - CLAUDE.md:702 ["proposed, not run"] `planted test for it is proposed, not run. **Limitations, stated because they bound the`
    - CLAUDE.md:730 ["considered and rejected"] `consumer saw the corrected labeling automatically. That approach was considered and rejected`
    - CLAUDE.md:839 ["diagnostic-only"] `measurable, phenomenon confirmed real and structural, no mechanism yet (diagnostic-only)`
    - CLAUDE.md:909 ["diagnostic-only"] `**No mechanism (in-loop or outer-loop) has been designed.** This ticket is diagnostic-only`
    - CLAUDE.md:910 ["not yet done"] `throughout, deliberately, matching how tickets 82/84 were sequenced. **Not yet done:** the`
    - CLAUDE.md:1145 ["not yet started"] `| 82 | M2, per-community domain balance | No mechanism currently constrains any individual community's semantic/social mix; only the global 50/50 reconstruction-loss weighting exists. v8's `binding_penalty` (per-communit`
    - CLAUDE.md:1146 ["no mechanism"] `| 86 | M2/M3, per-community mass ("ghost communities") | No mechanism exists to detect or address a community holding very low share of the model's total reconstructed mass — the third major open issue this session, alon`
    - CLAUDE.md:1236 ["not yet implemented"] `## 10. Planned Extensions (not yet implemented)`
    - CLAUDE.md:1273 ["diagnostic-only"] `the full corpus, not the toy corpus — diagnostic-only, no production wiring implied by`
    - CLAUDE.md:1300 ["considered and rejected"] `**Considered and rejected: an R²/explained-variance analog.** Since every relation is already`
    - CLAUDE.md:1315 ["status: proposal only"] `**Status: proposal only, not scheduled, not built.** No code exists for either (1) or (2).`
