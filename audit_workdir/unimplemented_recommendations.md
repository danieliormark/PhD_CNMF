...
- **This list has not yet been checked against current code for silent implementation** — the
  mirror-image check the original audit design called for (a recommendation could have been
  built since the sentence recommending it was written, without the doc being updated). None of
  the 15 above were spot-checked against `chunk13v9.py`/`diagnostic_scripts/` beyond what's
  already common knowledge from this session's other work. That is the natural next step if
  this list needs to be trusted as currently accurate, not just historically accurate.
